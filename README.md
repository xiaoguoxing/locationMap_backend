...
```cmd
01_extract_csv.bat
# Linux 下执行：./01_extract_csv.sh
```

按自然周（周一到周日）分组，最近 30 天约 5~6 个周 × 3 个参数 = **约 15~18 次查询**，相比滚动模式减少 **84%** 查询次数。

**7天滚动窗口模式（按需使用）**：

```cmd
01_extract_csv.bat --days 30 --range-days 7
# Linux 下执行：./01_extract_csv.sh --days 30 --range-days 7
```
...
该接口默认执行 `--weekly` 模式（等同于 `01_extract_csv.bat --weekly`）：最近 30 天、按自然周（周一到周日）分组，共约 5~6 个日期区间 × 3 项参数，约 15~18 次 SQL Server 查询（相比 7 天滚动窗口减少约 84% 查询次数）。前端只提交后台任务并显示提交结果，不轮询状态、不刷新 React Query（请求缓存）、不切换日期、不重建当前地图。新数据在用户下次刷新或重新进入页面后进入日期列表。
